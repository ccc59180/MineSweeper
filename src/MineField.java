...
public class MineField {
    ...
    public void populateMineField(int row, int col) {
        ...
        for (int i = 0; i < numMines; i++) {
            rowRandom = generator.nextInt(mineData.length);
            colRandom = generator.nextInt(mineData[0].length);
            while ((rowRandom == row && colRandom == col) || hasMine(rowRandom, colRandom)) {
                rowRandom = generator.nextInt(mineData.length);
                colRandom = generator.nextInt(mineData[0].length);
            }
            mineData[rowRandom][colRandom] = true;
        }
    }
    ...
    public int numAdjacentMines(int row, int col) {
        assert inRange(row, col) && !hasMine(row, col);
        ...
        return numAdjacentMine;       // DUMMY CODE so skeleton compiles
    }
